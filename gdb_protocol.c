...
static void cont(gdb_protocol_data_t gdb, char *data)
{
    ...
    {
        single_step(gdb);
        PCr = registers_read(gdb->reg, 15, registers_get_mode(gdb->reg));
        stop = (gdb_find_breakpoint(gdb, PCr) != -1);
        if (stop)
        {
...
static void write_register(gdb_protocol_data_t gdb, unsigned int reg, char *data)
{
    ...
    {
        warning("PC value is outside the address space, did you set the right endianess in gdb ?\n");
    }
